...
export const ResultDetail: React.FC<Props> = ({ result }) => {
  ...
  const applyInvoiceCodeFilter = useSearchStore(s => s.applyInvoiceCodeFilter);
  const applyInvoiceNumberSort = useSearchStore(s => s.applyInvoiceNumberSort);
  ...
  const derivedTaxAmount = useMemo(() => {
    return lineItems
      .filter(li => typeof li.tax_rate === 'number' && li.tax_rate > 0)
      .reduce((sum, li) => {
    ...
    <div className="px-4 pb-3 pl-[50px] bg-bg-secondary border-b border-border animate-detail-in">
      ...
      {isBank && (
        <table className="w-full border-collapse">
          <tbody>
            ...
            <EditableField label={t('invoice_code', 'Invoice Code')} value={result.invoice_code || ''} fieldName="invoice_code" tableName="bank_statement_data" recordId={result.id} override={getOverride('invoice_code')} ctrlClickTitle={result.invoice_code ? t('ctrl_click_filter_invoice_code', 'Ctrl+Click to filter by this invoice code') : undefined} onCtrlClick={result.invoice_code ? () => applyInvoiceCodeFilter(result.invoice_code) : undefined} onSave={(v) => handleSave('bank_statement_data', 'invoice_code', v)} onResolve={(a) => handleResolve('invoice_code', a)} />
            <EditableField label={t('invoice_number', 'Invoice #')} value={result.invoice_number || ''} fieldName="invoice_number" tableName="bank_statement_data" recordId={result.id} override={getOverride('invoice_number')} ctrlClickTitle={result.invoice_number ? t('ctrl_click_sort_invoice_number', 'Ctrl+Click to sort by invoice #') : undefined} onCtrlClick={result.invoice_number ? () => applyInvoiceNumberSort() : undefined} onSave={(v) => handleSave('bank_statement_data', 'invoice_number', v)} onResolve={(a) => handleResolve('invoice_number', a)} />
      ...
      {isInvoice && (
        <>
          <table className="w-full border-collapse">
            <tbody>
              <EditableField label={t('invoice_code', 'Invoice Code')} value={result.invoice_code || ''} fieldName="invoice_code" tableName="invoice_data" recordId={result.id} override={getOverride('invoice_code')} ctrlClickTitle={result.invoice_code ? t('ctrl_click_filter_invoice_code', 'Ctrl+Click to filter by this invoice code') : undefined} onCtrlClick={result.invoice_code ? () => applyInvoiceCodeFilter(result.invoice_code) : undefined} onSave={(v) => handleSave('invoice_data', 'invoice_code', v)} onResolve={(a) => handleResolve('invoice_code', a)} />
              <EditableField label={t('invoice_number', 'Invoice #')} value={result.invoice_number || ''} fieldName="invoice_number" tableName="invoice_data" recordId={result.id} override={getOverride('invoice_number')} ctrlClickTitle={result.invoice_number ? t('ctrl_click_sort_invoice_number', 'Ctrl+Click to sort by invoice #') : undefined} onCtrlClick={result.invoice_number ? () => applyInvoiceNumberSort() : undefined} onSave={(v) => handleSave('invoice_data', 'invoice_number', v)} onResolve={(a) => handleResolve('invoice_number', a)} />
