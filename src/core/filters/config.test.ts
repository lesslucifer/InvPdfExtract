...
describe('loadFilterConfig', () => {
  it('returns defaults when no config file exists', () => {
    const config = loadFilterConfig(tmpDir);
  ...
  it('preserves all default fields for a full config', () => {
...
describe('saveFilterConfig', () => {
  it('writes config to disk and round-trips correctly', () => {
    const custom = { ...DEFAULT_FILTER_CONFIG, skipThreshold: 0.3, aiTriageEnabled: false };

    saveFilterConfig(tmpDir, custom);

    const loaded = loadFilterConfig(tmpDir);
    expect(loaded.skipThreshold).toBe(0.3);
    expect(loaded.aiTriageEnabled).toBe(false);
  });

  it('writes valid JSON', () => {
    saveFilterConfig(tmpDir, DEFAULT_FILTER_CONFIG);
